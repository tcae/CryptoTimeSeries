...
function Base.show(io::IO, ohlcv::OhlcvData)
    print(io::IO, "ohlcv: base=$(ohlcv.base) interval=$(ohlcv.interval) size=$(size(ohlcv.df)) calculated intervals=$(size(ohlcv.df, 1) > 0 ? round(Int, (ohlcv.df[end, :opentime] - (ohlcv.df[begin, :opentime] - Dates.Minute(1)))/intervalperiod(ohlcv.interval)) : 0) start=$(size(ohlcv.df, 1) > 0 ? ohlcv.df[begin, :opentime] : "no start datetime)") end=$(size(ohlcv.df, 1) > 0 ? ohlcv.df[end, :opentime] : "no end datetime)") ix=$(ohlcv.ix)")
end

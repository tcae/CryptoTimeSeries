module TradeAuditTest

using Dates, JSON3, Test
using TradeAudit

function sampleevent()::TradeAudit.AuditEventRow
    return TradeAudit.AuditEventRow(
        event_id="evt-001",
...
@testset "TradeAudit tests" begin
    event = sampleevent()

    @test event.event_id == "evt-001"
    @test event.instrument_type == TradeAudit.spot_pair

    payload = TradeAudit.eventpayload(event)
    ...
    mktempdir() do root
        folder = TradeAudit.auditfolder(event; root=root)
        ...
        path = TradeAudit.writeevent(event; root=root)
        @test path == joinpath(folder, "events.jsonl")
        @test isfile(path)

        TradeAudit.writeevent(TradeAudit.AuditEventRow(event_id="evt-002", event_type=TradeAudit.ORDER_ACK, event_time_utc=event.event_time_utc, created_at_utc=event.created_at_utc, source_module="CryptoXch", environment="production", run_mode="live", exchange="KrakenSpot", account_alias="krakenspot-tcae1", routing_role=TradeAudit.routing_trade_exchange_spot, market_type=TradeAudit.market_spot, asset_class=TradeAudit.crypto, instrument_type=TradeAudit.spot_pair, symbol="BTCUSDT"); root=root)
    ...
    old_enabled = get(ENV, "CTS_AUDIT_ENABLED", nothing)
    old_sim_enabled = get(ENV, "CTS_AUDIT_SIMULATION_ENABLED", nothing)
    try
        ENV["CTS_AUDIT_ENABLED"] = "false"
        mktempdir() do root
            path = TradeAudit.writeevent(event; root=root)
            @test path == ""
        ...
        ENV["CTS_AUDIT_SIMULATION_ENABLED"] = "false"

        simulation_event = TradeAudit.AuditEventRow(
        ...
        mktempdir() do root
            path = TradeAudit.writeevent(simulation_event; root=root)
            @test path == ""
            @test isempty(readdir(root))
        end
    finally
        if isnothing(old_enabled)
            delete!(ENV, "CTS_AUDIT_ENABLED")
        else
            ENV["CTS_AUDIT_ENABLED"] = old_enabled
        end
        if isnothing(old_sim_enabled)
            delete!(ENV, "CTS_AUDIT_SIMULATION_ENABLED")
        else
            ENV["CTS_AUDIT_SIMULATION_ENABLED"] = old_sim_enabled
        end
    end
end

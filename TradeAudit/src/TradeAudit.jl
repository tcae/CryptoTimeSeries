"""
TradeAudit defines the canonical audit event schema and append-only JSONL writer
for order, execution, and portfolio events under the shared `\$HOME/crypto/audit`
folder hierarchy. Includes hash-chain manifest for integrity validation and Arrow
export for efficient columnar storage and analytics.
"""
module TradeAudit

using Arrow, Dates, EnvConfig, JSON3, SHA, UUIDs
...
"Return the canonical audit root folder under the shared crypto root."
function auditroot(root::Union{Nothing, AbstractString}=nothing)::String
    basepath = if isnothing(root)
        normpath(get(ENV, "CTS_AUDIT_ROOT", joinpath(EnvConfig.cryptopath, "audit")))
    else
...
function auditenabled(event::AuditEventRow)::Bool
    _envbool("CTS_AUDIT_ENABLED", true) || return false
    if lowercase(event.run_mode) == "simulation"
        _envbool("CTS_AUDIT_SIMULATION_ENABLED", true) || return false
    end
    return true
end
...
Compute SHA256 hash of an audit event payload for integrity validation.
Returns hex-encoded hash string.
"""
function computeevenhash(event::AuditEventRow)::String
    payload = eventpayload(event)
    payload_json = JSON3.write(payload)
    hash_bytes = SHA.sha256(payload_json)
...
Append one audit event to JSONL and update manifest with event hash chain.
Returns the written path. Hash chain enables integrity validation and tamper detection.
"""
function writeeventwithhash(event::AuditEventRow; root::Union{Nothing, AbstractString}=nothing)::String
    ...
    event_hash = computeevenhash(event)
    manifest_path = manifestfile(event; root=root)
    manifest = readmanifest(manifest_path)
    prior_hash = isempty(get(manifest, "event_hashes", [])) ? nothing : priorevenhash(manifest_path)
    
    # Record hash in manifest
    hashes = get!(manifest, "event_hashes", [])
    push!(hashes, Dict("event_id" => string(event.event_id), "hash" => event_hash))
    ...
    open(path, "a") do io
        ...
        payload["prior_event_hash"] = prior_hash
...
Validate integrity of audit event hash chain in a folder's JSONL file.
Returns (is_valid::Bool, issues::Vector{String}).
"""
function validatehashchain(folder::String)::Tuple{Bool, Vector{String}}
    issues = String[]
    events_jsonl = joinpath(folder, "events.jsonl")
    manifest_json = joinpath(folder, "manifest.json")
    
    !isfile(events_jsonl) && return (true, issues)
    !isfile(manifest_json) && push!(issues, "Manifest file missing: $manifest_json")
    
    manifest = readmanifest(manifest_json)
    recorded_hashes = Dict(get(manifest, "event_hashes", []))
    
    # Validate each event
    prior_hash = nothing
    line_num = 0
    for line in eachline(events_jsonl)
        line_num += 1
        isempty(strip(line)) && continue
        
        try
            payload = JSON3.read(line)
            event_id = get(payload, "event_id", "")
            event_hash = get(payload, "event_hash", "")
            recorded_prior = get(payload, "prior_event_hash", nothing)
            
            # Recompute hash (excluding hash fields)
            clean_payload = Dict{String, Any}()
            for (k, v) in pairs(payload)
                key = String(k)
                if !(key in ["event_hash", "prior_event_hash"])
                    clean_payload[key] = v
                end
            end
            computed_hash = bytes2hex(SHA.sha256(JSON3.write(clean_payload)))
            
            if computed_hash != event_hash
                push!(issues, "Line $line_num: Hash mismatch for event $event_id (computed=$computed_hash, recorded=$event_hash)")
            end
            
            if !isnothing(prior_hash) && recorded_prior != prior_hash
                push!(issues, "Line $line_num: Prior hash mismatch for event $event_id")
            end
            
            prior_hash = event_hash
        catch err
            push!(issues, "Line $line_num: JSON parse error: $err")
        end
    end

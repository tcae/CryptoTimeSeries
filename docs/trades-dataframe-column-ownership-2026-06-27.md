...
| `closereason` | Xch/TradingStrategy | Ensure Trades column `closereason` exists. Owner: Xch/TradingStrategy. Eltype: `CategoricalVector{String}`. Records the executed close reason (`none`, `stoploss`, `takeprofit`, `trendchange`, or `liquidation`) for replay gain compilation. An open position dropped at a partition end has no close reason. |

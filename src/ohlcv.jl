...
function addpivot!(ohlcv::OhlcvData)
    ohlcv.df.pivot = (ohlcv.df.open + ohlcv.df.high + ohlcv.df.low + ohlcv.df.close) ./ 4
    return ohlcv
end
...
function readcsv(base::String)::OhlcvData
    ...
    io = CSV.File(Config.datafile(mnemonic(ohlcv), "_df.csv"), types=Dict(1=>String, 2=>Float32, 3=>Float32, 4=>Float32, 5=>Float32, 6=>Float32, 7=>Float32))
    df = DataFrame(io)
...
function read(base::String)::OhlcvData
    ...
    # display(first(df, 1))
    println("loaded OHLCV data of $base from $(df[1, :opentime]) until $(df[end, :opentime]) with $(nrow(df)) rows")
    ohlcv = OhlcvData(df, base)

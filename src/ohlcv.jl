...
function defaultcrypto(base)::OhlcvData
    ohlcv = OhlcvData(DataFrame(), base, "usdt", "binance", "1m")
end
...
function read!(ohlcv::OhlcvData)::OhlcvData
    ...
    # display(first(df, 1))
    println("loaded OHLCV data of $(ohlcv.base) from $(df[1, :timestamp]) until $(df[end, :timestamp]) with $(nrow(df)) rows at $(ohlcv.interval) interval")
    setdataframe!(ohlcv::OhlcvData, df)

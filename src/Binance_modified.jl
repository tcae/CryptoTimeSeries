...
BINANCE_API_ACCOUNT = string(BINANCE_API_REST, "api/v3/account")
...
function getKlines(symbol; startDateTime=nothing, endDateTime=nothing, interval="1m")
    ...
    # display(length(arr))
    # res = [klines2j(arr[ix]) for ix in 1:length(arr)]
    # DataFrames.DataFrame(res)

    # [display(ix) for ix in arr]
    # klines2j(arr[1])
...
# "https://api.binance.com/api/v3/account?recvWindow=5000&timestamp=1611080662547&signature=3ec801e37ae078a87ef22527df306294222d7dfa013a4b1fff7730e491d3256e"
# "https://api.binance.com/api/v3/account?recvWindow=5000&timestamp=1611080814449&signature=da7b143e518bf97d7ff9fb874bc5e7535ea66306e0222c64c6a11c82de1ec313"
...
function account(apiKey::String, apiSecret::String)
    ...
    qstring = string(BINANCE_API_ACCOUNT, "?", query, "&signature=", doSign(query, apiSecret))
    # display(qstring)
    r = HTTP.request("GET", qstring, headers)
    ...
    #interval => 1m 3m 5m 15m 30m 1h 2h 4h 6h 8h 12h 1d 3d 1w 1M
      ...
      @async begin
        HTTP.WebSockets.open(string("wss://stream.binance.com:9443/ws/",join(allStreams, "/")); verbose=false) do io
            while !eof(io) && goahead
                data = String(readavailable(io))
                goahead = callback(data)
            end
        end
    end
end

...
@testset "Targets::Trend04 crosscheck mitigation tests" begin
    @testset "monotonic continuations keep buy extension" begin
        thres = Targets.LabelThresholds(0.025f0, 0.01f0, -0.01f0, -0.025f0)
        ...
        @test count(==(longopen), longtrd.df.label) > 0
        ...
        @test count(==(shortopen), shorttrd.df.label) > 0
        @test count(==(shorthold), shorttrd.df.label) == 0
    end

    @testset "state machine starts long at local low" begin
        thres = Targets.LabelThresholds(0.03f0, 0.005f0, -0.005f0, -0.03f0)
        ...
        firstlong = findfirst(in((longopen, longhold)), trd.df.label)
    ...
    @testset "state machine avoids hold-only short runs" begin
        thres = Targets.LabelThresholds(0.01f0, 0.005f0, -0.005f0, -0.01f0)
        trd = Targets.Trend04(2, 10, thres)
        ...
        @test (count(==(shorthold), labels) == 0) || (count(==(shortopen), labels) > 0)
        for ix in eachindex(labels)
            if labels[ix] == shorthold
                @test any(labels[1:ix] .== shortopen)
            end
        end
    end
    ...
    @testset "slow short continuation does not require per-window shortopen" begin
        thres = Targets.LabelThresholds(0.03f0, 0.005f0, -0.005f0, -0.03f0)
        trd = Targets.Trend04(2, 3, thres)

        labels = TradeLabel[shortopen, shortopen, shortopen, shortopen, shortopen]

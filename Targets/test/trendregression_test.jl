...
@testset "TrendRegression interface" begin
    ...
    @test Targets.uniquelabels(uptrd) == [Targets.longopen, Targets.shortopen, Targets.allclose]
    @test collect(Targets.labels(uptrd)) == [Targets.longopen, Targets.longopen, Targets.longopen, Targets.longopen, Targets.allclose]
    @test all(Targets.relativegain(uptrd, 1, 4) .> 0.04f0)
    @test Targets.relativegain(uptrd, 5, 5) == [0.0f0]
    @test collect(Targets.labelbinarytargets(uptrd, Targets.longopen)) == [true, true, true, true, false]
    @test collect(Targets.labelrelativegain(uptrd, Targets.longopen)) == collect(Targets.relativegain(uptrd)) .* Float32[1, 1, 1, 1, 0]
    ...
    @test collect(Targets.labels(downtrd)) == [Targets.shortopen, Targets.shortopen, Targets.shortopen, Targets.shortopen, Targets.allclose]
